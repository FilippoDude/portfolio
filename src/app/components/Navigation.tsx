...
const navigationButtons : NavigationButtonType[] = [
    ...
    {name: "Cooler 3d Portfolio (W.I.P)", link: "/cool", customLink: false},
    {name: "Widgets Crate", link: "https://widgetscrate.filippodude.cc/download", customLink: true},
    {name: "3D Demo", link: "/game", customLink: false},

]
const NavigationMenu = ({innerRef, hideMenu} : {innerRef: React.Ref<HTMLDivElement>, hideMenu: () => void}) => {
    return (
        <div ref={innerRef} className="w-screen md:w-70 h-full bg-[#42517340] backdrop-blur-2xl flex flex-col items-center pt-4 gap-1">
            {navigationButtons.map((e, i) => {
                return <div className="relative max-w-9/10 w-9/10 py-2" onClick={hideMenu} key={i}><Link href={e.link} key={i} className="cursor-pointer relative text-white z-60 flex items-center w-full h-full group">
            ...
            <div className=" absolute w-full bottom-0 left-0 flex md:hidden items-center flex-col gap-2 pb-2 backdrop-blur-xl">
                ...
                <button className="w-full h-full cursor-pointer" onClick={hideMenu}><p className="text-white text-2xl font-bold">Hide</p></button>
            </div>
        </div>
    )
}
...
const NavigationButton = () => {
    ...
    const hideMenu = (instant: boolean = false) => {
        if(ref.current == null || obscuratorRef.current == null || menuRef.current == null) return 
        document.body.style.overflow = ""
        let fullscreen = false
        if(containerRef.current != null && containerRef.current.clientWidth < 768) fullscreen = true
        gsap.to(ref.current,{
    ...
    const onMouseEnter = () => {
        if(containerRef.current != null && containerRef.current.clientWidth < 768) return
        showMenu()
    }
    ...
    return (
        <div ref={containerRef} className="fixed w-full h-full z-60 left-0 top-0 pointer-events-none">
            <div ref={obscuratorRef} className="bg-[#000000] opacity-0 w-full h-full pointer-events-none"/>
            <div ref={ref} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} className="absolute top-0 -left-999 flex flex-row h-full pointer-events-auto">
                <NavigationMenu innerRef={menuRef} hideMenu={hideMenu}></NavigationMenu>
                <button onClick={onClick} className={`mt-2 w-10 h-20 bg-[#42517340] rounded-r-2xl backdrop-blur-2xl cursor-pointer md:cursor-default`}>
